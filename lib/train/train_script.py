...
from CiteTracker.lib.train.actors import CiteTrackerActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "citetrack":
        net = build_citetrack(cfg)
    else:
    ...
    if settings.script_name == "citetrack":
        ...
        actor = CiteTrackerActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
