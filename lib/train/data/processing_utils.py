...
def sample_target_for_clip(im, target_bb, search_area_factor):
...
#     im_crop = im_crop[:, :, [2, 1, 0]]
#     PIL_image = Image.fromarray(im_crop)
...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if output_sz is not None:
        ...
        if mask is None:
            return im_crop_padded, resize_factor, att_mask, im_crop
        mask_crop_padded = \
        ...
        if mask is None:
            return im_crop_padded, att_mask.astype(np.bool_), 1.0, im_crop
        #  add im_crop
