    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                # target box for initial frame
                target_bb = template_anno['bbox'][0]
                template_array = template_frames[0]
                template_crop_PIL = sample_target_for_clip(template_array, target_bb, 2.0)
                newsize = (320, 320)
                template_clip = template_crop_PIL.resize(newsize)

                template_clip_tensor = transforms.ToTensor()(template_clip)
                template_clip_tensor = template_clip_tensor.unsqueeze(0)
                data.update({"template_clip": template_clip_tensor})

                # target box for search frame
                search_target_bb = search_anno['bbox'][0]
                search_array = search_frames[0]
                search_crop_PIL = sample_target_for_clip(search_array, search_target_bb, 2.0)
                newsize = (320, 320)
                search_clip = search_crop_PIL.resize(newsize)
                search_clip_tensor = transforms.ToTensor()(search_clip)
                search_clip_tensor = search_clip_tensor.unsqueeze(0)

                data.update({"search_clip": search_clip_tensor})
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            try:
                ...
                target_bb = template_anno['bbox'][0]
                template_array = template_frames[0]
                template_crop_PIL = sample_target_for_clip(template_array, target_bb, 3.0)
                newsize = (300, 300)
                template_clip = template_crop_PIL.resize(newsize)
